...
export async function getBitcoinNodeData(): Promise<Result> {
  ...
  if (total > 0) {
    result.knotsPercentage = parseFloat(
      ((result.knots / total) * 100).toFixed(2),
    );
    result.corePercentage = parseFloat(
      ((result.core / total) * 100).toFixed(2),
    );
    result.othersPercentage = parseFloat(
      ((result.others / total) * 100).toFixed(2),
    );
  }
